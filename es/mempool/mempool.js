...
const fetchAndUpdateBlockHeight = () => {
  fetch('https://mempool.space/api/blocks/tip/height')
    ...
    .then(blockHeight => {
      ...
      const texture = createTextTexture("Altura de Bloques", `${blockHeight}`);
...
const fetchAndUpdateMempoolCount = async () => {
  fetch('https://mempool.space/api/mempool/txids')
    ...
    .then(txids => {
      ...
      const texture = createTextTexture("Conteo Mempool", `${mempoolCount}`);
...
const fetchAndUpdateRecommendedFees = () => {
  fetch('https://mempool.space/api/v1/fees/recommended')
    ...
    .then(fees => {
      ...
      const feesText = `Alta: ${fees.fastestFee} \nBaja: ${fees.hourFee}`;
      const texture = createTextTexture("Tarifas - Sats/vB", feesText);
...
const fetchAndUpdateTimeAvg = () => {
  fetch('https://mempool.space/api/v1/difficulty-adjustment')
    ...
    .then(difficultyData => {
      ...
      const texture = createTextTexture("⏱ Promed Bloques", displayText);
